...
def table_regional_demand(df_supply, df_demand, discipline, status_list, avg_grad_class_size):
    ...
    df_step_demand = df_step_demand.groupby(['Region'])[['AvgAnnualOpenings_LT']].sum().reset_index()
    df_step_demand = df_step_demand.rename(columns={'AvgAnnualOpenings_LT': 'AvgAnnualOpenings'})
