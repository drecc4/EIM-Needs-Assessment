...
def app():
    ...
    #System Settings
    st.sidebar.write('**System Settings**')
    st.markdown(' ')

    fcast_range = st.sidebar.selectbox("Forecast Range", ('ST', 'LT'))
    ...
    #demand drivers
    # df_demand_detail = df_demand_detail.loc[df_demand_detail['Forecast'] == 'ST']
    pmp_fcast_year_start = df_demand_detail.BaseYear.max()
    pmp_fcast_year_end = df_demand_detail.ProjectedYear.max()


    #entire country (i.e. DPT in USA)
    df_demand_detail_discipline = df_demand_detail.loc[df_demand_detail['Discipline'] == professional_abbreviation]
    ...
    df_demand_detail_region = df_demand_detail.loc[df_demand_detail['Discipline'] == professional_abbreviation]
    df_demand_detail_region = df_demand_detail_region.loc[df_demand_detail_region['Region'] == program_campus_region]
    df_demand_detail_region = df_demand_detail_region.sort_values(by='EstJobChangePct', ascending=False)
    ...
    df_demand_detail_state = df_demand_detail.loc[df_demand_detail['Discipline'] == professional_abbreviation]
